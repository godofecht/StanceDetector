...
def processTwitterSourcePosts(twitter_dataset_path):
    ...
    for directory in twitter_dirs_sorted:
        for subdir in next(os.walk(twitter_dataset_path + '/' + directory))[1]:
          tweet_src_path = twitter_dataset_path + '/' + directory + '/' + subdir +'/source-tweet' #accessing source directories
          # twitter_src_dirs.append(next(os.walk(tweet_src_path))[2])
          twitter_src_dirs.append(tweet_src_path)
    ...
    for file in twitter_src_dirs:
        paths = file + '/' + file.split('/')[-2] + '.json'
        tweet_post_dict = {}
...
def processTwitterReplyPosts(twitter_dataset_path, twitter_dirs_sorted):
    ...
    for directory in twitter_dirs_sorted:
        for subdir in next(os.walk(twitter_dataset_path + '/' + directory))[1]:
            tweet_src_path = twitter_dataset_path + '/' + directory + '/' + subdir +'/replies' #accessing source directories
            # tweet_src_path = twitter_dataset_path + '/' + directory + '/replies'
            # print(tweet_src_path)
            replies_files.append(next(os.walk(tweet_src_path))[2])
            # print(replies_files)
            for i in (next(os.walk(tweet_src_path))[2]):
                  paths = twitter_dataset_path + '/' + directory + '/' + subdir +'/replies' + '/' + i #accessing source directories
                  # print(paths)
